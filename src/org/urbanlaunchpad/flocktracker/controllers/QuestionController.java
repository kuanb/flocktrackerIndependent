...
public class QuestionController {
	...
	private boolean inLoop = false; // Toggle that turns on if the survey gets
									// into a loop.
	private int loopPosition = -1; // Position in the questions array in the
	...
	public QuestionController(Context context,
			SubmissionHelper submissionHelper, Metadata metadata, Bus eventBus) {
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				new ColumnCheckHelper(chapterList, trackingQuestions)
						.runChecks();
			}
		}).run();
	}
	...
	private void startLoop() {
		loopPosition = 0;
		loopIteration = 0;
		loopTotal = Integer.parseInt(getCurrentQuestionFragment()
				.getSelectedAnswers().iterator().next());
	...
	public void showCurrentQuestion() {
		...
		case MULTIPLE_CHOICE:
			currentQuestionFragment = new MultipleChoiceQuestionFragment(
					currentQuestion, QuestionUtil.getQuestionPositionType(
							currentQuestion, chapterList.length), eventBus);
			break;
		case LOOP:
		case OPEN_NUMBER:
		case OPEN_TEXT:
			currentQuestionFragment = new OpenQuestionFragment(currentQuestion,
					QuestionUtil.getQuestionPositionType(currentQuestion,
							chapterList.length), eventBus);
			break;
		case IMAGE:
			currentQuestionFragment = new ImageQuestionFragment(
					currentQuestion, QuestionUtil.getQuestionPositionType(
							currentQuestion, chapterList.length), eventBus);
			break;
		case CHECKBOX:
			currentQuestionFragment = new CheckBoxQuestionFragment(
					currentQuestion, QuestionUtil.getQuestionPositionType(
							currentQuestion, chapterList.length), eventBus);
			break;
		case ORDERED:
			currentQuestionFragment = new OrderedListQuestionFragment(
					currentQuestion, QuestionUtil.getQuestionPositionType(
							currentQuestion, chapterList.length), eventBus);
			break;
		}
		...
		if (isAskingTripQuestions) {
			if (inLoop) {
				currentQuestion = trackingQuestions[trackerQuestionPosition]
						.getLoopQuestions()[loopPosition];
				currentQuestion.initializeLoop(loopTotal, loopIteration,
						loopPosition);
			} else {
				currentQuestion = trackingQuestions[trackerQuestionPosition];
			}
	...
	public void switchToNextQuestion() {
		...
		if (!inLoop
				...
				&& !((String) currentQuestionFragment.getSelectedAnswers()
						.toArray()[0]).isEmpty()) {
			// If this is the question that has looped questions, initialize the
			// loop.
			...
			if (loopPosition == numLoopQuestions - 1) {
				// We have finished all iterations of the loop questions.
				// Continue to next
				// question.
				if (loopIteration == loopTotal - 1) {
					...
					loopIteration++;
					loopPosition = 0;
					getCurrentQuestion().updateLoopInfo(loopIteration,
							loopPosition);
					showCurrentQuestion();
					return;
				}
				...
				loopPosition++;
				getCurrentQuestion()
						.updateLoopInfo(loopIteration, loopPosition);
				showCurrentQuestion();
				return;
			}
		}
	...
	public void updateSurveyPosition(int chapterPosition, int questionPosition) {
		if (this.chapterPosition != chapterPosition
	...
	private void resetSurvey() {
		...
		// parse json survey
		try {
			surveyJSONObject = new JSONObject(ProjectConfig.get()
					.getOriginalJSONSurveyString());
	...
	public void resetTrip() {
		...
		trackingQuestions = JSONUtil.parseTrackingQuestions(context,
				surveyJSONObject);
		trackerQuestionPosition = 0;
		isAskingTripQuestions = false;
	}
